...
/// 用量 / 模型 / 用户偏好 REST 类接口固定使用的 IDE 版本。
///
/// 新版 IDE 对这些接口的 profileArn 行为更严格；Kiro.rs v0.6.0 验证过 0.9.2
/// 仍可稳定查询 Enterprise/IdC 账号的用量和模型信息。
...
fn cooldown_remaining_ms(until: Option<Instant>) -> Option<u64> {
    until
...
fn external_idp_discovery_url(issuer_url: &str) -> anyhow::Result<String> {
    let trimmed = issuer_url.trim();
    if trimmed.is_empty() {
        bail!("External IdP 刷新需要 issuerUrl");
    }

    let mut url = reqwest::Url::parse(trimmed)
        .with_context(|| format!("External IdP issuerUrl 无效: {}", trimmed))?;
...
async fn resolve_external_idp_token_endpoint(
    ...
    let issuer_url = credentials
        .issuer_url
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("External IdP 刷新需要 issuerUrl"))?;
    let discovery_url = external_idp_discovery_url(issuer_url)?;
    ...
    let response = client
        ...
        .send()
        .await
        .context("External IdP discovery 请求失败")?;

    let status = response.status();
    if !status.is_success() {
        let body_text = response.text().await.unwrap_or_default();
        bail!("External IdP discovery 失败: {} {}", status, body_text);
    }

    let data: ExternalIdpDiscoveryResponse = response
        .json()
        .await
        .context("解析 External IdP discovery 响应失败")?;
    if data.token_endpoint.trim().is_empty() {
        bail!("External IdP discovery 响应缺少 token_endpoint");
    }
    Ok(data.token_endpoint)
}

/// 刷新 External IdP Token（客户 IdP OIDC refresh_token grant）
async fn refresh_external_idp_token(
    ...
    let response = client
        ...
        .send()
        .await
        .context("External IdP Token 刷新请求失败")?;
    ...
    let data: ExternalIdpRefreshResponse = response
        .json()
        .await
        .context("解析 External IdP Token 响应失败")?;
...
async fn fetch_enterprise_profile_arn(
    ...
    for region in candidates.iter() {
        ...
        if status.is_success() {
            ...
            if let Some(arn) = data.first_arn() {
        ...
        let body_text = response.text().await.unwrap_or_default();
        tracing::warn!(
            "Enterprise ListAvailableProfiles 在 {} 失败: {} {}",
            region,
            status,
            body_text
        );
    ...
    tracing::warn!(
        "Enterprise ListAvailableProfiles 全部失败: {}",
        last_error.as_deref().unwrap_or("无可用端点")
    );
    bail!(
        "获取 Enterprise profileArn 失败: {}",
        last_error.unwrap_or_else(|| "无可用端点".to_string())
    )
}
...
pub struct CredentialEntrySnapshot {
    ...
    /// 普通 429 限流冷却剩余毫秒数；冷却中且 `> 0` 才返回。
    #[serde(skip_serializing_if = "Option::is_none")]
    pub rate_limited_for_ms: Option<u64>,
    /// 普通 429 限流冷却剩余毫秒数的兼容别名。
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct MultiTokenManager {
    ...
    /// Enterprise profileArn 自动获取失败后，本进程内不重复探测同一凭据。
    enterprise_profile_fetch_attempted: Mutex<HashSet<u64>>,
}
...
impl MultiTokenManager {
    ...
    pub fn new(
        ...
        let entries: Vec<CredentialEntry> = credentials
            ...
            .map(|mut cred| {
                ...
                if cred
                    .fill_default_profile_arn_with_region(Some(config_ref.effective_api_region()))
                {
                    has_new_ids = true;
                }
        ...
        let manager = Self {
            ...
            enterprise_profile_fetch_attempted: Mutex::new(HashSet::new()),
        };
    ...
    pub fn update_global_config_runtime(&self, config: Config) {
    ...
    /// 获取所有未禁用凭据中最短的普通 429 冷却剩余时间。
    pub fn next_rate_limit_delay(&self) -> Option<StdDuration> {
        self.entries
            .lock()
            .iter()
            .filter(|e| !e.disabled)
            .filter_map(|e| {
                e.rate_limited_until
    ...
    async fn try_ensure_token(
        ...
        let creds = self.ensure_enterprise_profile_arn(id, creds, &token).await;
    ...
    async fn ensure_enterprise_profile_arn(
        ...
        if !credentials.should_fetch_enterprise_profile_arn() {
            return credentials;
        }

        {
            let mut attempted = self.enterprise_profile_fetch_attempted.lock();
            if !attempted.insert(id) {
                return self
                    .ensure_enterprise_fallback_profile_arn(id, credentials)
                    .unwrap_or_else(|creds| creds);
            }
        }

        let global_proxy = self.proxy.lock().clone();
        let effective_proxy = credentials.effective_proxy(global_proxy.as_ref());
        match fetch_enterprise_profile_arn(
            &credentials,
            &self.config(),
            token,
        ...
        {
            Ok(Some(profile_arn)) => {
                tracing::info!("凭据 #{} 已自动获取 Enterprise profileArn", id);
                self.set_profile_arn_for(id, profile_arn)
            ...
            Ok(None) => self
                ...
                .unwrap_or_else(|creds| creds),
            Err(err) => {
                self.enterprise_profile_fetch_attempted.lock().remove(&id);
                tracing::warn!(
                    "凭据 #{} 自动获取 Enterprise profileArn 失败，本次保持原凭据并允许后续重试: {}",
                    id,
                    err
                );
                credentials
    ...
    fn ensure_enterprise_fallback_profile_arn(
        &self,
        id: u64,
        mut credentials: KiroCredentials,
    ) -> Result<KiroCredentials, KiroCredentials> {
        let fallback =
            enterprise_fallback_profile_arn(Some(credentials.effective_api_region(&self.config())));
        let needs_update = credentials.profile_arn.as_deref().is_none_or(|arn| {
            is_placeholder_profile_arn(arn)
                || (is_enterprise_fallback_profile_arn(arn) && arn != fallback)
        });
        ...
        credentials.profile_arn = Some(fallback.clone());
        Ok(self
            .set_profile_arn_for(id, fallback)
            .unwrap_or(credentials))
    }

    fn set_profile_arn_for(&self, id: u64, profile_arn: String) -> Option<KiroCredentials> {
        ...
        if changed && let Err(err) = self.persist_credentials() {
            tracing::warn!("profileArn 自愈后持久化失败（不影响本次请求）: {}", err);
        }
            ...
            if current_id == id
                && let Some(next) = entries
                    ...
                    .filter(|e| {
                        !e.disabled
                            && e.id != id
                            && cooldown_remaining_ms(e.throttled_until).is_none()
                            && cooldown_remaining_ms(e.rate_limited_until).is_none()
                    })
                    .min_by_key(|e| e.credentials.priority)
            {
    ...
    pub async fn add_credential(&self, new_cred: KiroCredentials) -> anyhow::Result<u64> {
        ...
        let new_cred_is_external_idp = new_cred.is_external_idp_credential();
        if new_cred_is_external_idp {
            validated_cred.auth_method = Some("external_idp".to_string());
        } else if let Some(auth_method) = new_cred.auth_method {
            validated_cred.auth_method = Some(
                    ...
                    || auth_method.eq_ignore_ascii_case("iam")
                {
                    ...
                    || auth_method.eq_ignore_ascii_case("externel_idp")
                    || auth_method.eq_ignore_ascii_case("externel-idp")
                    || auth_method.eq_ignore_ascii_case("externelIdp")
                {
        ...
        if new_cred.provider.is_some() {
            validated_cred.provider = new_cred.provider;
        }
        validated_cred
            .fill_default_profile_arn_with_region(Some(self.config().effective_api_region()));
        ...
        validated_cred.login_hint = new_cred.login_hint;
    ...
    pub fn update_auth_metadata(&self, id: u64, metadata: KiroCredentials) -> anyhow::Result<()> {
        let mut changed = false;
        {
            ...
            macro_rules! update_opt {
                ($field:ident) => {
                    if let Some(value) = metadata.$field.as_ref() {
                        if entry.credentials.$field.as_deref() != Some(value.as_str()) {
                            entry.credentials.$field = Some(value.clone());
                            changed = true;
                        }
                    }
                };
            ...
            update_opt!(auth_method);
            update_opt!(provider);
            update_opt!(client_id);
            update_opt!(client_secret);
            update_opt!(start_url);
            update_opt!(issuer_url);
            update_opt!(token_endpoint);
            update_opt!(scopes);
            update_opt!(audience);
            update_opt!(login_hint);
            update_opt!(region);
            update_opt!(auth_region);
            update_opt!(api_region);
            update_opt!(profile_arn);
        }
...
mod tests {
    ...
    #[test]
    fn test_validate_refresh_token_allows_short_external_idp_token() {
        let mut credentials = KiroCredentials::default();
        credentials.auth_method = Some("externel_idp".to_string());
        credentials.refresh_token = Some("short-refresh".to_string());
    ...
    #[test]
    fn test_external_idp_discovery_url() {
        assert_eq!(
            external_idp_discovery_url("https://idp.example.com").unwrap(),
            "https://idp.example.com/.well-known/openid-configuration"
        );
        assert_eq!(
            external_idp_discovery_url("https://idp.example.com/oauth2/default").unwrap(),
    ...
    #[test]
    fn test_list_available_profiles_first_arn_skips_blank() {
    ...
    fn test_retry_policy_presets_and_validation() {
        let fast = RetryPolicy::preset(RetryMode::Fast);
        assert_eq!(fast.rate_limit_cooldown_ms, 3_000);
        assert_eq!(fast.max_request_retries, 9);
        assert!(fast.credential_switch_on_429);
        assert!(!fast.respect_retry_after);

        let polite = RetryPolicy::preset(RetryMode::Polite);
        assert_eq!(polite.rate_limit_cooldown_ms, 60_000);
        assert_eq!(polite.max_request_retries, 4);
        assert!(!polite.credential_switch_on_429);
        assert!(polite.respect_retry_after);

        let custom = RetryPolicy {
            rate_limit_cooldown_ms: 800,
        ...
        assert!(RetryPolicy::effective(RetryMode::Custom, None).is_err());
    }
    ...
    fn test_report_rate_limited_skips_credential_without_disabling() {
        let mut cred1 = KiroCredentials::default();
        cred1.id = Some(1);
        cred1.kiro_api_key = Some("ksk_rate_limit_1".to_string());
        cred1.auth_method = Some("api_key".to_string());
        cred1.priority = 1;

        let mut cred2 = KiroCredentials::default();
        cred2.id = Some(2);
        cred2.kiro_api_key = Some("ksk_rate_limit_2".to_string());
        ...
        assert!(first.rate_limited_for_ms.unwrap_or_default() > 0);
    ...
    fn test_set_retry_policy_persists_to_config_file() {
        ...
        let mut config = Config::default();
        config.set_config_path_for_test(path.clone());
        config.save().unwrap();

        let manager = MultiTokenManager::new(config, vec![], None, None, true).unwrap();
        let custom = RetryPolicy {
            rate_limit_cooldown_ms: 800,
            max_request_retries: 15,
            base_backoff_ms: 80,
