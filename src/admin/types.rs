...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RuntimeCompatibilityResponse {
    pub default_endpoint: String,
    pub tool_compatibility_mode: String,
    pub endpoint_compatibility_version: String,
}
...
pub struct UpdateGlobalConfigRequest {
    ...
    #[serde(default)]
    pub tool_compatibility_mode: Option<String>,
    #[serde(default)]
...
pub struct RetryPolicyResponse {
    /// 当前模式（turbo/fast/balanced/steady/polite/custom）
    pub mode: String,
    /// 自定义策略（仅 custom 模式使用）
