...
use crate::model::config::{Config, RetryMode, ToolCompatibilityMode};
...
fn parse_tool_mode(value: &str) -> Result<ToolCompatibilityMode, AdminServiceError> {
    match value.trim() {
        "claude-code" => Ok(ToolCompatibilityMode::ClaudeCode),
        "raw" => Ok(ToolCompatibilityMode::Raw),
        other => Err(AdminServiceError::InvalidCredential(format!(
            "toolCompatibilityMode 必须是 claude-code 或 raw，收到: {}",
            other
        ))),
    }
}

fn tool_mode_to_string(mode: ToolCompatibilityMode) -> String {
    match mode {
        ToolCompatibilityMode::ClaudeCode => "claude-code".to_string(),
        ToolCompatibilityMode::Raw => "raw".to_string(),
    }
}
...
fn credential_to_kam_account(cred: KiroCredentials) -> Option<KamExportAccount> {
    ...
    Some(KamExportAccount {
        ...
        issuer_url: non_empty(cred.issuer_url),
        token_endpoint: non_empty(cred.token_endpoint),
        scopes: non_empty(cred.scopes),
        audience: non_empty(cred.audience),
        login_hint: non_empty(cred.login_hint),
...
impl AdminService {
    ...
    /// 注入 KiroProvider，用于运行时热更新全局代理和默认 endpoint。
    ...
    pub fn get_all_credentials(&self) -> CredentialsStatusResponse {
        ...
        let mut credentials: Vec<CredentialStatusItem> = snapshot
            ...
            .map(|entry| {
                ...
                let configured_endpoint = entry.endpoint;
                let effective_endpoint = configured_endpoint
                    .clone()
    ...
    pub fn set_global_proxy(&self, url: Option<String>) -> Result<(), AdminServiceError> {
        ...
        if let Some(provider) = &self.kiro_provider
            && let Err(e) = provider.update_global_proxy(proxy)
        {
    ...
    pub fn get_global_config(&self) -> GlobalConfigResponse {
        ...
        GlobalConfigResponse {
            ...
            auth_region: cfg.auth_region.clone(),
            api_region: cfg.api_region.clone(),
            default_endpoint: cfg.default_endpoint.clone(),
            tool_compatibility_mode: tool_mode_to_string(cfg.tool_compatibility_mode),
            trace_enabled: self
                .trace_store
                .as_ref()
                .map(|s| s.is_enabled())
            ...
            proxy_url: self.get_global_proxy(),
            known_endpoints: self.sorted_endpoints(),
        }
    }

    /// 更新全局配置，运行时生效并持久化到 config.json。
    pub fn update_global_config(
        &self,
        req: UpdateGlobalConfigRequest,
    ) -> Result<GlobalConfigResponse, AdminServiceError> {
        if req.region.is_none()
            && req.auth_region.is_none()
            && req.api_region.is_none()
            && req.default_endpoint.is_none()
            && req.tool_compatibility_mode.is_none()
            && req.trace_enabled.is_none()
            && req.trace_retention_days.is_none()
            && req.usage_log_retention_days.is_none()
            && req.proxy_url.is_none()
        {
            return Err(AdminServiceError::InvalidCredential(
                "至少提供一个要更新的配置字段".to_string(),
            ));
        }

        let mut cfg = self.token_manager.config();
        ...
        if let Some(auth_region) = req.auth_region {
            cfg.auth_region = normalize_optional_string(auth_region);
        }
        if let Some(api_region) = req.api_region {
            cfg.api_region = normalize_optional_string(api_region);
        }
        if let Some(endpoint) = req.default_endpoint {
            let trimmed = endpoint.trim();
            if trimmed.is_empty() {
                return Err(AdminServiceError::InvalidCredential(
                    "defaultEndpoint 不能为空".to_string(),
                ));
            }
            ...
            endpoint_changed = cfg.default_endpoint != trimmed;
            cfg.default_endpoint = trimmed.to_string();
        }
        if let Some(mode) = req.tool_compatibility_mode {
            cfg.tool_compatibility_mode = parse_tool_mode(&mode)?;
        }
        if let Some(days) = req.trace_retention_days {
            validate_retention_days("traceRetentionDays", days)?;
            cfg.trace_retention_days = days;
        }
        if let Some(days) = req.usage_log_retention_days {
            validate_retention_days("usageLogRetentionDays", days)?;
            cfg.usage_log_retention_days = days;
        }
        if let Some(enabled) = req.trace_enabled {
            cfg.trace_enabled = enabled;
        }
        if let Some(proxy_url) = req.proxy_url {
            let normalized = normalize_optional_string(proxy_url);
            validate_proxy_url(normalized.as_deref())?;
        ...
        if endpoint_changed {
            if let Some(provider) = &self.kiro_provider
                && let Err(e) = provider.update_default_endpoint(cfg.default_endpoint.clone())
            {
                return Err(AdminServiceError::InternalError(format!(
                    "热更新默认 endpoint 失败: {}",
                    e
        ...
        if let Some(s) = &self.trace_store {
            s.set_enabled(cfg.trace_enabled);
            s.set_retention_days(cfg.trace_retention_days);
        }
        if let Some(r) = &self.usage_recorder {
            r.set_retention_days(cfg.usage_log_retention_days as i64);
        }
    ...
    fn sorted_endpoints(&self) -> Vec<String> {
        let mut endpoints: Vec<String> = self.known_endpoints.iter().cloned().collect();
        endpoints.sort();
        endpoints
    }
    ...
    /// 获取当前运行兼容配置
    pub fn get_runtime_compatibility(&self) -> crate::admin::types::RuntimeCompatibilityResponse {
        let cfg = self.token_manager.config();
        crate::admin::types::RuntimeCompatibilityResponse {
            default_endpoint: cfg.default_endpoint.clone(),
            tool_compatibility_mode: tool_mode_to_string(cfg.tool_compatibility_mode),
            endpoint_compatibility_version: "cli-2.6.0;ide-0.12.301".to_string(),
        }
    }
    ...
    async fn do_complete_social_login(
        ...
        if let Some(target_id) = session.relogin_target_id {
            ...
            self.do_relogin_update(target_id, refresh_token, session.cred_template)
                .map_err(|e| AdminServiceError::InternalError(e.to_string()))?;
    ...
    pub async fn start_idc_login(
        ...
        let cred_template = KiroCredentials {
            ...
            provider: if start_url.trim_end_matches('/') != BUILDER_ID_START_URL {
                Some("IAM_SSO".to_string())
            } else {
                None
            },
            ...
            idc::PollResult::Success(token) => {
                ...
                if let Some(target_id) = relogin_target_id {
                    if let Some(refresh_token) = token.refresh_token {
                        self.do_relogin_update(target_id, refresh_token, cred_template)
                            .map_err(|e| AdminServiceError::InternalError(e.to_string()))?;
                    }
    ...
    fn do_relogin_update(
        &self,
        target_id: u64,
        refresh_token: String,
        metadata: KiroCredentials,
    ) -> anyhow::Result<()> {
        ...
        self.token_manager
            .update_auth_metadata(target_id, metadata)?;
        // 重置失败计数并重新启用
